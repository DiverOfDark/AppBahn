...
`appbahn login` authenticates against the platform and stores a short-lived token;
the landing page explains what AppBahn is to someone who has never seen it.

### What shipped

- `appbahn` Cobra-based CLI with `login`, `workspace`, `project`, `env`, `resource`,
  `deploy`, `member`, `token`, and `version` command groups.
